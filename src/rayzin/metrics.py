...
class EuclideanMetric:
    ...
    def lower_bounds(
        self, queries: Float32Array, centroids: Float32Array, radii: Float32Array
    ) -> Float32Array:
        deltas = np.asarray(
...
class CosineMetric:
    ...
    def lower_bounds(
        ...
        distances = np.asarray(self.pairwise(centroids, queries), dtype=np.float32).T
        if distances.ndim != 2:
            msg = (
                f"Expected pairwise distances to have shape (n_rows, nq), got {distances.shape!r}."
            )
...
def add_lower_bounds_fn(
    ...
    lower_bounds = metric.lower_bounds(
        query_matrix,
        centroids,
        radii,
    )
